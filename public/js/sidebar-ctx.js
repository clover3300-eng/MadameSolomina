...
(function () {
    ...
    function itemHtml(it) {
        ...
        // ДОХОДНОСТЬ ЗА ВСЁ ВРЕМЯ — ПИЛЮЛЕЙ, А НЕ ЦВЕТНЫМ ТЕКСТОМ (2026-08-04).
        // Голое «+12,4%» зелёным в колонке навигации читалось как случайно
        // подкрашенная подпись; пилюля с тоном знака — это уже число со своим
        // местом, тем же языком, что дельта на карточке портфеля.
        if (it.ret && it.ret.tx) {
            right += '<span class="sbc-ret' + (it.ret.neg ? ' neg' : '') + '">' + esc(it.ret.tx) + '</span>';
        } else if (it.val) right += '<span class="sbc-val">' + esc(it.val) + '</span>';
        ...
        var acts = '';
        if (it.hide) {
            var hTitle = it.hide.off ? 'Вернуть портфель в учёт: карточка и деньги снова в виджетах'
                : 'Убрать портфель из учёта: карточка уйдёт с «Обзора», а его деньги — из сумм и графиков';
