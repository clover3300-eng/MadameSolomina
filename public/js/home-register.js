...
/* ============================================================================
   Свайп вниз на Главной → открыть «Рынок». Работает и на тач-устройствах
   (палец вниз), и на десктопе (колесо/трекпад вниз). Жест активен ТОЛЬКО пока
   активна вкладка Главная, чтобы не мешать прокрутке контента других вкладок.
   ========================================================================== */
(function () {
    function homeActive() {
        var p = document.getElementById('panel-home');
    ...
    // --- Тач: свайп пальцем вниз ---
    var tsY = 0, tsX = 0, tracking = false;
    document.addEventListener('touchstart', function (e) {
        tracking = homeActive() && e.touches.length === 1;
        if (!tracking) return;
        tsY = e.touches[0].clientY;
        tsX = e.touches[0].clientX;
    }, { passive: true });
    document.addEventListener('touchend', function (e) {
        if (!tracking || !homeActive()) { tracking = false; return; }
        tracking = false;
        var t = e.changedTouches[0];
        var dy = t.clientY - tsY;
        var dx = t.clientX - tsX;
        // Явно вертикальный свайп вниз, с запасом по длине
        if (dy > 70 && dy > Math.abs(dx) * 1.4) goMarket();
    }, { passive: true });

    // --- Десктоп: колесо/трекпад вниз ---
    var wheelAcc = 0, wheelTimer = null, wheelLock = false;
    window.addEventListener('wheel', function (e) {
        if (wheelLock || !homeActive() || e.deltaY <= 0) return;
        wheelAcc += e.deltaY;
        clearTimeout(wheelTimer);
        wheelTimer = setTimeout(function () { wheelAcc = 0; }, 220);
        if (wheelAcc > 110) {
            wheelAcc = 0;
            wheelLock = true;          // один переход на жест
            goMarket();
