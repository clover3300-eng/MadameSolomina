...
(function () {
    ...
    window.pfBrokerPfImport = function () {
        ...
        if (exist) {   // карточка уже есть: синк сейчас + показать её, вторую не плодим
            if (!connAlive()) {
                toast('Брокер не подключён на этом устройстве — карточка показывает последний сохранённый снимок счёта');
    ...
    window.pfBrokerPfRefresh = function (pid, ev) {
        ...
        sync(true, function (err) {
            ...
            if (err) { toast((err && err.message) || 'Не удалось обновить счёт', true); return; }
            toast('Счёт обновлён · ' + timeStr(live.ts));
            // одна кнопка — одно понятное действие: обновляем и позиции, и
            // историю сделок, иначе для второго пришлось бы заводить вторую
            opsSync(true);
        });
    };
    ...
    PF.brokerPfBadgeHtml = function (p) {
        ...
        return '<span class="pfbk-badge" title="' + attr('Портфель счёта «' + accName + '» — наполняется из Т-Инвестиций сам, примерно раз в минуту.') + '">' +
            ...
            '<button class="pfbk-re" type="button" onclick="pfBrokerPfRefresh(\'' + p.id + '\',event)" aria-label="Обновить из Т-Инвестиций" title="Обновить сейчас: позиции счёта и историю сделок">' + RELOAD_SVG + '</button></span>';
    };
    ...
    // Тихая строка про историю сделок (этап 2). Молчит, когда всё в порядке:
    // разобранная история и так видна настоящими датами в составе и в журнале,
    // а рапорт об успехе в карточке — лишний шум. Говорит только о том, что
    // пользователю стоит знать: идёт разбор, история неполная или не далась.
    // Ручного пере-разбора здесь нет намеренно — его делает ↻ на бейдже.
    function opsNoteHtml(p) {
        var o = p.broker.ops;
        if (!connFor(p)) return '';                  // чужое устройство/счёт — про это говорит бейдж
        if (opsLive.busy) return '<div class="pfbk-extra pfbk-ops">разбираем историю сделок со счёта…</div>';
        if (opsLive.err && !o) {
            return '<div class="pfbk-extra pfbk-ops err" title="' + attr(opsLive.err +
                ' Кнопка ↻ на бейдже карточки пробует ещё раз.') + '">историю сделок загрузить не удалось</div>';
        }
        if (!o) return '';                           // разбор вот-вот пойдёт сам — молчим
        if (o.covered >= o.total) return '';         // всё покрыто — говорить не о чем
        // часть бумаг историей не покрыта: у метода брокера есть потолок в
        // тысячу операций, плюс бывают переводы бумаг и корпоративные действия
        var left = o.total - o.covered;
        return '<div class="pfbk-extra pfbk-ops" title="' + attr('У ' + left + ' из ' + o.total +
            ' бумаг покупки в истории счёта не нашлись — брокер отдаёт только последнюю тысячу операций, ' +
            'а переводы бумаг и корпоративные действия покупками не считаются. ' +
            'Для таких бумаг дата покупки условная, количество и стоимость — верные.') + '">' +
            'история сделок неполная · ' + o.covered + ' из ' + o.total + ' ' +
            PF.plural(o.total, 'бумаги', 'бумаг', 'бумаг') + '</div>';
    }
})();
