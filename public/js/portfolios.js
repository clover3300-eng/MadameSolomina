...
(function () {
    ...
    window.pfImport = function (source, sub, pid, name) {
        ...
        PF.openMenu = null; ensureQuotes(true); renderPortfolios(); autofillNkd(holds); toast('Импортировано: ' + holds.length);
    };
    ...
    window.pfImportData = function (input) {
        ...
        reader.onload = function () {
            ...
            pfConfirm({
                ...
                saveStore(); PF.openMenu = null; ensureQuotes(true); renderPortfolios();
    ...
    window.pfSetColor = function (pid, col) {
        ...
        p.color = col; PF.colorsOpen = false; saveStore(); renderPortfolios();
    };
    ...
    window.pfDelArm = function (on) { PF.delArm = !!on; renderPortfolios(); };
    window.pfDeleteYes = function (pid) {
        ...
        PF.delArm = false; renderPortfolios(); toast('Портфель удалён');
    };
    ...
    window.pfRemoveHolding = function (pid, hid) {
        ...
        delete PF.editHold[hid]; saveStore(); pfInvalidateCharts(pid); renderPortfolios();
    };
    ...
    window.pfAddLot = function (pid, hid) {
        ...
        PF.editHold[hid] = true; saveStore(); pfInvalidateCharts(pid); renderPortfolios();
    };
    ...
    window.pfAddToggle = function (pid) {
        PF.addOpen = !PF.addOpen; renderPortfolios();
    ...
    window.pfColorsToggle = function () { PF.colorsOpen = !PF.colorsOpen; renderPortfolios(); };
    document.addEventListener('click', function (e) {
        if (!PF.colorsOpen) return;
        if (e.target && e.target.closest && e.target.closest('.pfm-colorwrap')) return;
        PF.colorsOpen = false; renderPortfolios();
    });
    ...
    PF.CHECK_SVG = CHECK_SVG; PF.CHEV_SVG = CHEV_SVG; PF.EYEOFF_SVG = EYEOFF_SVG; PF.EYE_SVG = EYE_SVG;
        PF.PFDGRID_SVG = PFDGRID_SVG;     PF.UNDO_SVG = UNDO_SVG; PF.XMARK_SVG = XMARK_SVG; PF.cardHtml = cardHtml; PF.closeImpMenus = closeImpMenus;
    PF.pfCardHead = pfCardHead;
    PF.pfConfirm = pfConfirm;                                     PF.plural = plural;     PF.renderSmooth = renderSmooth; PF.summaryCardHtml = summaryCardHtml;
    ...
    PF.REBAL_SVG = REBAL_SVG; PF.allHiddenHtml = allHiddenHtml; PF.emptyHtml = emptyHtml; PF.menuHtml = menuHtml;
