...
(function () {
    ...
    function sizeBtnHtml(p, cur) {
        ...
        return '<span class="pfc-size" title="' + attr('Размер карточки: S — до сводки, M — со сводкой, L — всё до «Ребаланса». ' +
            'Выбор разворачивает блок под этот размер целиком. ' +
    ...
    window.pfCardSize = function (pid, s, ev) {
        ...
        if (b.cfg.sz[b.id] === s) delete b.cfg.sz[b.id];
            ...
            b.cfg.sz[b.id] = s;
            // ВЫСОТА БЛОКА СЛЕДУЕТ ЗА ВЫБРАННЫМ РАЗМЕРОМ. Заданную вручную высоту
            // снимаем — блок разворачивается ровно под карточку выбранного размера.
            // Иначе выходило то, на что жаловались: в низком блоке (S) клик по M/L
            // менял набор этажей, но не высоту, и сводка ложилась на график.
            // Зеркальное правило ресайза уже есть (тяга за кромку снимает ручной
            // размер, см. onUp в portfolios-dash.js): у каждого жеста один смысл —
            // тянешь кромку → высота ведёт размер, жмёшь букву → размер ведёт высоту.
            if (b.cfg.h) delete b.cfg.h[b.id];
            if (b.cfg.hm) delete b.cfg.hm[b.id];
        }
        saveDashCfg();
    ...
    function menuHtml(p) {
        ...
        var foot = '<div class="pfm-bottom">' +
            '<div class="pfm-foot">' +
                ...
                '<button class="pfm-quiet" onclick="pfToggleHidden(\'' + p.id + '\')" title="Убрать портфель из учёта: карточка уйдёт с «Обзора», деньги — из сумм и графиков. Вернуть — глазом на строке портфеля в сайдбаре">' +
    ...
    window.pfToggleHidden = function (pid, ev) {
        ...
        // чтобы «скрыть» не читалось как пропажа вкладки. 2026-08-04: скрытие
        // стало выводом ИЗ УЧЁТА (деньги уходят из сумм и графиков) — тост
        // говорит об этом прямо, «Вернуть» тут же под рукой.
        var tabKept = p.hidden && pfxWide() && pfxOpenPfTabs.indexOf(pid) >= 0;
        if (p.hidden) {
            toast('Портфель «' + p.name + '» убран из учёта — его деньги не входят в суммы и графики' +
                (tabKept ? '; вкладка осталась' : ''), false,
