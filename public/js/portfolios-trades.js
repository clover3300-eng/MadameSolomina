...
(function () {
    ...
    function tradeRowHtml(t, multiPf) {
        ...
        return '<div class="pft-row' + (t.rebal ? ' pft-row-rebal' : '') + '">' +
            // ПРИНАДЛЕЖНОСТЬ — точка цвета портфеля И ЕГО ИМЯ (мокап overview3, экран 15).
            // Был номер-маркер без подписи: цифру в цветном пятне опознаёт только тот, кто
            // уже выучил палитру, а имя пряталось в подсказке — то есть при беглом чтении
            // строки отличались друг от друга ничем. Колонка появляется только при двух
            // и более портфелях: сообщать принадлежность, когда выбора нет, незачем.
            (multiPf ? '<span class="pft-pf" style="--c:' + t.pfColor + '" title="' + esc(t.pfName) + '">' +
                '<i class="pft-pfdot"></i><b class="pft-pfnm">' + esc(t.pfName) + '</b></span>' : '') +
