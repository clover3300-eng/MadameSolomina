...
(function () {
    ...
    function applyAuthView(showDone) {
        ...
        // Кикер («что это вообще») нужен только гостю: вошедший уже знает,
        // куда пришёл, и строка превращается в шум над приветствием.
        var title = el('hcTitle'), lead = el('hcLead'), cta = el('hcCtaLabel');
        var kick = el('hcKick');
        if (kick) kick.hidden = showDone;
        if (title && lead) {
            if (showDone) {
                ...
                title.innerHTML = 'Что будет с вашими деньгами <em>через три года</em>';
                lead.textContent = 'Соберём портфель из ОФЗ и акций по ценам Мосбиржи, ' +
                    'покажем прогноз, выплаты и точный список к покупке.';
            }
        }
    }
