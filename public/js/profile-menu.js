...
(function () {
    ...
    // Полноэкранный терминал прячет шапку сайта вместе с аватаром, а личный
    // кабинет там нужен (макет 01). Отдаём ему ровно две вещи: чем подписать
    // свою кнопку и как открыть ту же панель — второй копии профиля не заводим.
    window.pmIdentity = function () {
        var p = getProfile();
        return { ini: initials(), photo: (p && p.photo) || '', guest: !p };
    };
    window.pmOpenHub = function () { try { openHub(); } catch (e) {} };
