...
(function () {
    ...
    // ---- ПОЛ ВЫСОТЫ КАРТОЧКИ ПОРТФЕЛЯ = её размер S ---------------------------
    // Карточка портфеля — единственный блок, у которого от высоты зависит НАБОР
    // этажей (cardSizeOf: S/M/L), а самый низкий из них всё-таки не резиновый:
    // герой + пилюля + кривая + подпись занимают 318–324px на любой ширине блока
    // (замерено на span 3/4/12). Ниже этого сжимать уже нечего — блок начинал
    // резать сам S, и сводка ложилась на график («наложение данных»). Поэтому
    // 324 — общий пол: и для тяги за кромку, и для высот из старых раскладок.
    var PFD_PF_MIN_H = 324;
    // заданная высота блока с учётом этого пола (ноль = высота авто, по содержимому)
    function pfdHOf(id) {
        var h = +((PF.dashCfg.h || {})[id]) || 0;
        if (!h || String(id).indexOf('pf:') !== 0) return h;
        // Размер карточки выбран ВРУЧНУЮ (ховер-кнопка S/M/L) → высота блока АВТО:
        // размер ведёт блок, а не наоборот. Свежий выбор чистит cfg.h сам
        // (pfCardSize), а здесь отваливаются высоты из раскладок, сохранённых ДО
        // этого правила, — иначе старый дашборд так и остался бы с обрезанной M.
        if ((PF.dashCfg.sz || {})[id]) return 0;
        return Math.max(h, PFD_PF_MIN_H);
    }
    ...
    var PFX_TAB_SEEDS = {
        ...
        // «Мои заявки» — спокойной полосой снизу. Раньше три карточки стояли в ряд,
        // а график шёл под ними во всю ширину и читался пустой полосой.
        // График остаётся defHidden как БЛОК (его копии 2..4 включают руками), но
    ...
    function pfxTabSeed(tab) {
        ...
        (PFX_TAB_SEEDS[tab] || []).forEach(function (r) {
            cfg.order.push(r[0]); cfg.col[r[0]] = r[1]; cfg.span[r[0]] = r[2]; cfg.hidden[r[0]] = 0;
            if (r[3]) cfg.h[r[0]] = r[3];   // высота из сида — необязательный 4-й элемент
        });
        return cfg;
    }
    ...
    // R9.3: сюда же — виджеты, чей правый угол занят СВОЕЙ кнопкой-переходом или «+».
    // Раньше их спасали адресные сдвиги оверлея в px (у «Карты рынка» — right:144/174),
    // но отступ от правого края не зависит от ШИРИНЫ блока: на узкой карточке (222px)
    // обе кнопки уезжали прямо на заголовок. Кнопки в потоке шапки такого не умеют.
    var PFD_OWN_CHROME = { plist: 1, pdetail: 1, heat: 1, idx: 1, fav: 1, news: 1, rates: 1, calm: 1,
        // 2026-08-04: график капитала (обе версии) — правый угол занят пилюлями
        // периода и кнопкой «Сравнить», угловой оверлей ложился прямо на них
        cap: 1, cap2: 1,
        'trade:ob': 1, 'trade:ticket': 1, 'trade:orders': 1, 'trade:chart': 1, 'trade:pos': 1 };
    ...
    function pfdBodyHtml(favStr, noBonds) {
        ...
        var items = shown.map(function (b) {
            ...
            var h = pfdHOf(b.id);   // с полом карточки портфеля (PFD_PF_MIN_H)
    ...
    document.addEventListener('pointerdown', function (e) {
        ...
        // Карточка портфеля: пол выше — её размер S (PFD_PF_MIN_H). Ниже него блок
        // резал бы уже самый компактный набор этажей, и подпись графика/сводка
        // наезжали на кривую. Ограничение натуральной высотой то же, что выше:
        // у карточки пустого портфеля она может быть меньше — снап в авто остаётся.
        if (String(id).indexOf('pf:') === 0) minH = Math.max(minH, Math.min(PFD_PF_MIN_H, natH));
        ...
        function onMove(ev) {
            ...
            // masonry: соседи переезжают под новый размер. СИНХРОННО, а не через
            // rAF (pfdRepackSoon) — фикс наложения 2026-07-30. Высота блока в
            // masonry живёт не в height, а в grid-row span (строка = 1px), и
            // между «поставили height» и «пересчитали span» блок физически выше
            // своей брони: при тяге ВНИЗ он наезжал на нижнего соседа ровно на
            // столько, на сколько его вытянули (замерено — до 509px). rAF этот
            // разрыв не закрывает: pointermove успевает прийти уже ПОСЛЕ кадрового
            // колбэка, и наложение доживало до следующего кадра — на глаз
            // «данные накладываются друг на друга». Пак дешёвый (13 блоков),
            // синхронный вызов здесь — та же манера, что в pfdSchedulePack при
            // рендере и в pfdFlip при реордере.
            pfdPack();
        }
        function onUp() {
            cleanup();
            ...
            if (hMode && newH) {
                // Карточка портфеля: ручная тяга высоты — намерение СИЛЬНЕЕ прежнего клика по
                // переключателю S/M/L, поэтому ручной размер снимаем и он снова следует за
                // блоком. Иначе выходил тупик: растянул блок из S вниз — карточка осталась
                // маленькой, под ней мёртвое поле, и «размер больше не меняется»
                // (замечание 2026-07-30).
                if (PF.dashCfg.sz) delete PF.dashCfg.sz[id];
