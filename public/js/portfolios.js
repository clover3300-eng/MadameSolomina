...
(function () {
    ...
    PF.softRerender = softRerender; PF.paintPfChartMini = paintPfChartMini;
    PF.pfdCapMaybeRepaint = pfdCapMaybeRepaint; PF.donutHtml = donutHtml;
    PF.potentialOf = potentialOf; PF.favTickers = favTickers;
    ...
    window.pfEyeOpenTab = function (pid, ev) {
        ...
        if (pfxOpenPfTabs.indexOf(pid) < 0) { pfxOpenPfTabs.push(pid); pfxSaveOpenTabs(); }
        if (PF.pfxTab === 'pf:' + pid) { renderNoAnim(); return; }   // уже там — просто закрыть попап рендером
        window.pfxGoTab('pf:' + pid);
    };
    ...
    function pfwPlistHtml() {
        ...
        return '<div class="dash2-card pf-card2 pf-plistblk">' +
            pfCardHead('', 'Список портфелей', 'всего ' + PF.store.items.length + ' ' + plural(PF.store.items.length, 'портфель', 'портфеля', 'портфелей') + ' · ' + fmtRub(total),
    ...
    function pfxTabPortsHtml() {
        var vis = visibleItems();
        if (!vis.length) return PF.store.items.length ? allHiddenHtml() : emptyHtml();
    ...
    window.pfAddPortfolio = function () {
        if (PF.store.items.length >= MAX_CARDS) { toast('Максимум ' + MAX_CARDS + ' ' + plural(MAX_CARDS, 'портфель', 'портфеля', 'портфелей') + ' на странице', true); return; }
    ...
    window.pfImport = function (source, sub, pid, name) {
        ...
        if (PF.store.items.length >= MAX_CARDS) { toast('Максимум ' + MAX_CARDS + ' ' + plural(MAX_CARDS, 'портфель', 'портфеля', 'портфелей'), true); return; }
    ...
    window.pfToggleMenu = function (pid) {
        if (openMenu === pid) { openMenu = null; }
            ...
            openMenu = pid; menuJustOpened = true; PF.chartOpen = {}; PF.chartAssets = {}; PF.chartAssetsFull = {}; PF.holdsExpand = {};
    ...
    window.pfToggleChart = function (pid) {
        ...
        else { PF.chartOpen = {}; PF.chartAssets = {}; PF.chartAssetsFull = {}; PF.holdsExpand = {}; PF.chartOpen[pid] = true; openMenu = null; }
        renderPortfolios();
    ...
    window.pfOpenChartAssets = function (pid) {
        ...
        else { PF.chartOpen = {}; PF.chartAssets = {}; PF.chartAssetsFull = {}; PF.holdsExpand = {}; PF.chartOpen[pid] = true; PF.chartAssets[pid] = true; openMenu = null; }
        renderPortfolios();
        if (PF.chartOpen[pid]) loadPfChart(pid);
    };
    ...
    window.pfToggleHolds = function (pid) {
        ...
        else { PF.holdsExpand = {}; PF.holdsExpand[pid] = true; openMenu = null; PF.chartOpen = {}; PF.chartAssets = {}; PF.chartAssetsFull = {}; }
    ...
    PF.NOTE_CHECK_SVG = NOTE_CHECK_SVG; PF.NOTE_CLOCK_SVG = NOTE_CLOCK_SVG; PF.NOTE_ICON_SVG = NOTE_ICON_SVG; PF.NOTE_TRASH_SVG = NOTE_TRASH_SVG;
    PF.PFCM_WD = PFCM_WD; PF.PFDGRID_SVG = PFDGRID_SVG; PF.PFP_SLIDERS_SVG = PFP_SLIDERS_SVG; PF.PFX_TABS = PFX_TABS;
    PF.UNDO_SVG = UNDO_SVG; PF.XMARK_SVG = XMARK_SVG; PF.cardHtml = cardHtml; PF.closeImpMenus = closeImpMenus;
    PF.collectUpcomingPayouts = collectUpcomingPayouts; PF.newsHtmlCache = newsHtmlCache; PF.paymentCalendarHtml = paymentCalendarHtml; PF.pfCardHead = pfCardHead;
    PF.pfConfirm = pfConfirm; PF.pfcmCardHtml = pfcmCardHtml; PF.pfd2 = pfd2; PF.pfdAllocCompute = pfdAllocCompute;
    PF.pfdAllocHtml = pfdAllocHtml; PF.pfdAllocScope = pfdAllocScope; PF.pfdCapChartHtmlB = pfdCapChartHtmlB; PF.pfdCapChartHtml = pfdCapChartHtml;
    PF.pfdCapRepaint = pfdCapRepaint; PF.pfdCapSeries = pfdCapSeries; PF.pfdFlushNotes = pfdFlushNotes; PF.pfdHeatHtml = pfdHeatHtml;
    PF.pfdHeatRepaintSoon = pfdHeatRepaintSoon; PF.pfdKpiHtml = pfdKpiHtml; PF.pfdNewsHtml = pfdNewsHtml; PF.pfdNewsList = pfdNewsList;
    PF.pfdNoteHtml = pfdNoteHtml; PF.pfdPanelActive = pfdPanelActive; PF.pfwAssetsHtml = pfwAssetsHtml; PF.pfwConcHtml = pfwConcHtml;
    PF.pfwDivsHtml = pfwDivsHtml; PF.pfwIdxHtml = pfwIdxHtml; PF.pfwMoversHtml = pfwMoversHtml; PF.pfwOpsHtml = pfwOpsHtml;
    ...
    PF.pfwReportsHtml = pfwReportsHtml; PF.pfwSnapsHtml = pfwSnapsHtml; PF.pfwYieldHtml = pfwYieldHtml; PF.pfxBgRowHtml = pfxBgRowHtml;
    PF.pfxCornerRowHtml = pfxCornerRowHtml; PF.pfxFabSeen = pfxFabSeen; PF.pfxSetCardHtml = pfxSetCardHtml; PF.pfxTabPortsHtml = pfxTabPortsHtml;
    PF.pfxVisRowsHtml = pfxVisRowsHtml; PF.pfxWide = pfxWide; PF.plural = plural; PF.ratesHtml = ratesHtml;
    PF.ratesStackHtml = ratesStackHtml; PF.renderPosNews = renderPosNews; PF.renderSmooth = renderSmooth; PF.summaryCardHtml = summaryCardHtml;
